...
| U5 | The IFC is **+Z up** and its producer writes the semantics the engine reads (`IfcRelAggregates` on stairs, `OverallWidth` on doors, typed products) | A Y-up file voxelizes the building lying on its side; a semantically thin file converts *without error* into a subtly unwalkable world | `scripts/check_ifc_contract.py` measures both before conversion; `REVITER.md` has the frame-by-frame chain and the full input contract |
...
| D5 | `OverallWidth` tells the leaf count: **`round(OverallWidth / pitch)`**, capped by the opening's width in cells | Missing attribute → falls back to a single leaf, so entrance banks narrow. A width within ~0.1 cell of a half-cell boundary flips between one leaf and two, and a producer that derives `OverallWidth` from a bounding box reports a diagonal rather than a width (see `REVITER.md`) | Overrides: `{"leaves": 2}`; `check_ifc_contract.py` counts the missing and the boundary-fragile populations |
