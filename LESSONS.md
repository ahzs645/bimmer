...
| S20 | **`--rectify` leaves the glazing behind.** Audited floor by floor against the recovered model's own architectural plan, the wing moves leave 485 broken joins and 153 clashes over ten levels, and **433 of those 638 findings (68%) are Curtain Wall Mullions and Curtain Panels** -- at the same plan position on storey after storey | `wall_plan` builds every wing hull from `IfcWall` + `IfcWallStandardCase` placements. A curtain wall is neither, so it never enters the hull: the wall behind the glazing rotates 32 degrees and the glazing stays where it was, driven through the rooms that moved. This is also the unexplained 5% drop in glass voxels under `--rectify` that the per-triangle fix (S17) recovered none of -- there was nothing in the hull to cut | `adjacency_claims`: an element the hull missed travels with the wing when it TOUCHES something the wing claimed, bounded twice -- contact, and ALL of the element within reach of the hull (the box's FARTHEST corner, or a 40 m corridor that reaches a wing at one end is dragged whole into it: 138 see-through cells on a fixture that had none). Reachable interior 90.1% -> **95.3%**, beating the faithful build for the first time; largest stranded pocket 1,376 -> 293 cells. The boundary MOVES rather than vanishes -- findings within 2 m of the hull edge go 321 -> 26 and those more than 5 m out go 56 -> 357, which is the reach itself, and total findings 638 -> 526 with the curtain share 68% -> 57% (one decode, one flag: `--no-contact`). The model has no wing structure to use instead (one IfcBuilding, no zones, no assemblies), so a wing is inferred and an inferred boundary breaks joins somewhere. NB the audit only exists because the plan was drawn in a renderer that draws buildings -- the stick diagram of wall footprints could not show a wall crossing a room, because it did not draw rooms |
