...
| S21 | **A hull applied 87 m from the wing it was computed from still squares something, and the drawing looks right.** Two rounds of plan-side audit numbers were published from it before a walk found the error | `export-ifc.ts` writes tessellated coordinates RAW and puts the model's `origin` on the shared placement, so a consumer reading the IFC in world coordinates sees `feet * 0.3048 + origin`. `rectify-plan.ts` asserted in a comment that "the only conversion between the two is a scale". On this model the origin is (-0.46, +87.57) m. The hull then claims whatever is 87 m north of the wing -- and a big building has something there to claim | Pass the origin: `toFeet(input, result.origin)`, with a test whose hull only contains the wall once the origin comes off. **A frame is not verified by the output looking plausible.** Verify it against a quantity that cannot lie about position -- the model's own bbox. The mesh spans y -187..187 m and the IFC -99.7..278.3: same 378 m, 87.6 m apart, and that comparison costs one line |
