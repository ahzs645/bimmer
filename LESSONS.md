...
| S20 | **`--rectify` leaves the glazing behind.** Audited floor by floor against the recovered model's own architectural plan, the wing moves leave 485 broken joins and 153 clashes over ten levels, and **433 of those 638 findings (68%) are Curtain Wall Mullions and Curtain Panels** -- at the same plan position on storey after storey | `wall_plan` builds every wing hull from `IfcWall` + `IfcWallStandardCase` placements. A curtain wall is neither, so it never enters the hull: the wall behind the glazing rotates 32 degrees and the glazing stays where it was, driven through the rooms that moved. This is also the unexplained 5% drop in glass voxels under `--rectify` that the per-triangle fix (S17) recovered none of -- there was nothing in the hull to cut | `adjacency_claims`: an element the hull missed travels with the wing when it TOUCHES something the wing claimed, bounded twice -- contact, and ALL of the element within reach of the hull (the box's FARTHEST corner, or a 40 m corridor that reaches a wing at one end is dragged whole into it: 138 see-through cells on a fixture that had none). Reachable interior 90.1% -> **95.3%**, beating the faithful build for the first time; largest stranded pocket 1,376 -> 293 cells. The boundary MOVES rather than vanishes -- findings within 2 m of the hull edge go 321 -> **1** and those more than 5 m out go 56 -> 519, which is the reach itself, while total findings move only 638 -> 580 and clashes RISE 153 -> 167. The residue is relocated, not removed, and the curtain share falls 68% -> 54% (one decode, ONE wings file, one flag: `--no-contact`). The model has no wing structure to use instead (one IfcBuilding, no zones, no assemblies), so a wing is inferred and an inferred boundary breaks joins somewhere. NB the audit only exists because the plan was drawn in a renderer that draws buildings -- the stick diagram of wall footprints could not show a wall crossing a room, because it did not draw rooms |
| S21 | **A `ConvertResult` holds geometry in TWO frames 87 m apart, and which one you are touching decides whether the model's `origin` comes off.** Getting it wrong either way lands the wing hull 87 m from the wing it was computed from — where a 375 m building still has something to claim and square, so the drawing looks right | `export-ifc.ts` writes tessellated MESH coordinates raw and puts `origin` on the shared placement, so an IFC world coordinate is `feet * 0.3048 + origin`. But `elementBounds` — and the `solid`, `loops` and `boundsFeet` the architectural plan is drawn from — are ALREADY in that frame. Measured: `meshes` span y −187.6..187.5 m, `elementBounds` −99.7..274.9 m, and `origin.y` is +87.57. Reasoning from the exporter alone, I "fixed" the plan path (which was right), withdrew a correct published table and deleted its figure — then the re-render showed the wings NOT squaring | Pass the origin explicitly and let each caller decide: `rectifyForPlan` (elementBounds) passes zero, `rectify-walk.ts` (meshes) passes `result.origin`; a test pins BOTH directions, because a test for one is what let the other through. **A frame is never verified by the output looking plausible, and never by reading the writer's code** — verify it against a quantity that cannot lie about position, the model's own bbox, which is one line. And a withdrawal is an edit like any other: re-measure before publishing one, or it costs more than the error did |
| S22 | **An A/B test whose two halves came from different inputs.** The published contact-claim ablation paired `--no-contact` against the flag ON -- and, invisibly, wings computed from the Autodesk IFC against wings computed from the RECOVERED IFC. Both are legitimate inputs, so both columns looked right | The two runs were made in different sessions, hours apart, and the wings file was an argument I retyped rather than a fixed input I recorded. The mismatch only surfaced because an unrelated probe printed `moved` and it disagreed with the published number by 209 -- so I went looking for non-determinism in the decoder (there is none: two decodes agree to the element id sum) and found the argument instead | Corrected, the claim is worth 638 -> 580, not 638 -> 526, and clashes RISE. Run both halves of an ablation from ONE process and ONE input set -- `ablate.ts` decodes once and loops over the flag -- and validate the mirror by checking that one half reproduces the CLI exactly. **State the fixed inputs in the caption, not just the flag that varied**; "same decode, same wings, one flag apart" is a claim, and it has to be true |
| S23 | **Both ways of "extending the wall pattern" to close the rectification seam are refuted by measurement, and the second one had good evidence for it** | Extending a cut wall run along its own axis reaches its moved half in **23 of 412** broken-join pairs (6%); the median gap after the move is 25 m, because a wing ROTATES about its seam and the far end travels tens of metres -- nothing was cut, one side was carried away. The stronger idea looked well-supported: 63% of the residue is joined to its moved partner at 0 or 90 degrees (76% for `Walls`), i.e. it sits on the wing's OWN structural grid and only the hull's convexity excluded it, while the 40 m corridor the 6 m reach exists to refuse is at 32 degrees. So gate the claim on the angle family and relax the distance. Built, tested, measured: findings go 580 -> 639 -> 715 -> 805 -> 847 as the gated reach goes 9 -> 12 -> 18 -> 30 m | **Every element claimed beyond 6 m costs two to four broken joins**, because an element just outside the hull has most of its neighbours on the side it is ON and one contact with the side that moved: claiming it trades one broken join for several. The metric bound was not a crude proxy a structural test improves on -- it was already at the break-even point. Reverted. **A statistic about the RESIDUE is not a statistic about the population a rule would admit**: 63% of what is left behind being on-grid says nothing about what share of on-grid elements outside the reach belong to the wing. The real cause is upstream of both ideas: a RIGID transform applied to part of a building must break something at its edge, and only a non-rigid one (blend the rotation to identity across a transition band, so seam walls deform instead of breaking) can close that -- at the cost of straight walls near the seam |
| S24 | **A continuous transform does not save a join if the field STRAINS.** Replacing the wing hull's step with a smooth ramp -- the only fix S23 left open -- is correct, continuous and exact inside, and it still breaks more adjacencies than the rigid transform it replaces | Scored on all 169,116 pairs of structural elements that TOUCHED in the source model (the floor audit cannot judge this: "was it moved" is a category only a step function has, so under a continuous field those counts fall for free). Rigid hull-only breaks 2,229 and keeps 60.3% of runs on the grid; elastic breaks 5,009 / 7,867 / 9,381 / 12,131 / 14,158 at bands of 5 / 10 / 20 / 40 / 80 m, with on-grid falling 58.8% -> 22.1% and p99 strain at 244% for the narrowest band. The plan drawing shows it without the numbers: corridors smeared into tapered ribbons, plates dragged into wedges | **Strain is the displacement divided by the band width**, and a wing's far end has moved tens of metres, so holding strain under 10% needs a band the size of the building. Two elements half a foot apart in a field straining 100% end up a foot apart -- continuity is not sufficient, SMALL STRAIN is, and small strain is not on offer. I proposed this as "walls near the seam deform instead of breaking", which treats deformation as free; past a magnitude, deformation IS breaking. Kept behind `--band-metres`, off by default, for exactly one open question: a plate STRETCHED across a seam is walkable where a torn one is a canyon, so it may still buy reachable interior on the voxel side, which is a different measurement |
| S25 | **A gap that was only ever a way of counting.** "94 slabs against 107 -- floor/landing recovery remains incomplete" was carried as the project's one HIGH severity contract item for weeks. There is no missing floor plate | `check_floor_plates` counted `IfcSlab` ENTITIES, which is not comparable between producers in either half: Autodesk decomposes a Revit Roof into an `IfcRoof` container plus N `IfcSlab(.ROOF.)` parts sharing the parent's Tag (161 entities for 107 elements) and splits a ramp landing out as a slab, where the recovery writes one product per element. Joined on `Tag` across all four floor classes: all 94 match, none spurious, the 13 unmatched are 12 Revit Roofs and one ramp landing, Revit Floors are 68 of 68, and the recovery has 182 floor-class elements against 172 | Count by `Tag` across `IfcSlab` + `IfcCovering` + `IfcRoof` + `IfcRamp`. Then confirm WITHOUT the join, because a join can be wrong too: rasterise every near-horizontal face at 0.1 m into half-metre bands and 99.92% of the other producer's standable surface is reproduced within half a metre -- 87 sq m of 103,935 genuinely absent. **A count that cannot survive a change of producer is not a contract check**, and this one generated a high-severity item that nothing ever re-ran. That is why the self-tests are in CI now |
| S26 | **`by_type` returns SUBTYPES, so a type concatenated with its own subtype is a double count -- and a census error reaches the geometry through a threshold** | `by_type("IfcWall") + by_type("IfcWallStandardCase")` counted 7,381 of 7,521 walls twice; every document said "14,902 walls". A duplicated point is already in the hull of itself, so no hull moved directly. It doubled `n`, hence the share-derived family and wing thresholds, hence `overlap()` scored against a FIXED shove penalty | Deduplicate by id. Same six wings, same rotations, same pivots; wing 5's shove goes (0,+5) to (-1,+1) m, so the world changes and had to be REBUILT rather than assumed: reachable 95.3% -> 96.2%, cut off 1,866 -> 1,496, nothing measurably worse. Found as an incidental while measuring a different idea that did not pay; the refuted experiment earned its cost on this alone |
