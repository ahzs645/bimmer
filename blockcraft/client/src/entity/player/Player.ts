...
class Player {
  ...
  // Mode
  mode = "creative"; // bimmer: default to creative so you can fly around the building
  god = false;
  ...
  join(data) {
    ...
    // Spawn (bimmer: hover directly ABOVE the building's centre, looking down —
    // the building is centred on the origin, ~44 blocks tall at 0.5 m)
    this.respawn(world.blockSize, new THREE.Vector3(0, world.blockSize * 110, 0));
    ...
    // bimmer: spawn in creative and already flying so you don't fall
    this.mode = "creative";
    this.fly = true;
  ...
  placeBlock() {
    ...
    if (this.closest.point && this.place) {
      ...
      // bimmer: right-click a door to open/close it (toggles both halves)
      const doorToggle = { door_x: "door_x_open", door_x_open: "door_x", door_z: "door_z_open", door_z_open: "door_z" };
      const doorName = world.blockOrder[voxel - 1];
      if (doorToggle[doorName]) {
        let dx = Math.floor(innerPos.x),
          dy = Math.floor(innerPos.y),
          dz = Math.floor(innerPos.z);
        let to = world.blockId[doorToggle[doorName]];
        let cells = [[dx, dy, dz]];
        for (let ny of [dy + 1, dy - 1]) {
          let nv = world.getVoxel(dx, ny, dz);
          if (nv > 0 && doorToggle[world.blockOrder[nv - 1]]) {
            cells.push([dx, ny, dz]);
            break;
          }
        }
        for (let c of cells) {
          world.setVoxel(c[0], c[1], c[2], to);
          updateVoxelGeometry(c[0], c[1], c[2], true);
          g.socket.emit("setBlock", { x: c[0], y: c[1], z: c[2], t: to, class: "block" });
        }
        audioManager.play("dig.stone");
        this.place = false;
  ...
  collideVoxel(x, y, z) {
    let voxel = world.getVoxel(x, y, z);
    let n = world.blockOrder[voxel - 1];
    if (voxel > 1 && voxel != 255 && n != "door_x_open" && n != "door_z_open") return voxel; // bimmer: open doors are walk-through
  }
